...
func SplitInstalments(total entity.Money, n int) ([]entity.Money, error) {
	...
	parts := make([]entity.Money, 0, n)
	for i := 0; i < n; i++ {
		amountMinor := base
		if int64(i) < extra {
			amountMinor = amountMinor.Add(step)
		}
		parts = append(parts, entity.NewMoney(amountMinor.Mul(unit), ccy))
	}
