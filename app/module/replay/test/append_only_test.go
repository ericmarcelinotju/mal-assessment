...
func TestAppendOnly_LogIsMonotonic(t *testing.T) {
	...
	t.Run("when the log is read twice then it only grows", func(t *testing.T) {
		cfg := config.Default()
		s := newStack(t, cfg.FeeReversal, replay.CanonicalAccounts()...)

		var lengths []int
		for day := entity.Day(1); day <= 6; day++ {
			for _, ev := range replay.CanonicalStream() {
				if ev.PostingDay == day {
					_, _ = s.replay.Post(context.Background(), ev)
				}
			}
			assert.NoError(t, s.replay.CloseDay(context.Background(), day))
			lengths = append(lengths, len(entries(t, s)))
		}
...
func TestAppendOnly_CallerCannotRewriteHistory(t *testing.T) {
	t.Run("when a caller mutates the returned slice then the ledger is unaffected", func(t *testing.T) {
		...
		stolen := entries(t, s)
		before := row(t, s, replay.ACC001, 6).ClosingBalance

		for i := range stolen {
			stolen[i].Amount = aed("999999.00")
		}

		assertMoney(t, before, row(t, s, replay.ACC001, 6).ClosingBalance,
	...
	t.Run("when accruals are read then they are a copy too", func(t *testing.T) {
		...
		stolen := accruals(t, s)
		before := row(t, s, replay.ACC001, 2).InterestAccrued
		for i := range stolen {
			stolen[i].Amount = aed("42.00")
		}

		assertMoney(t, before, row(t, s, replay.ACC001, 2).InterestAccrued, "")
	})
}
...
func TestAppendOnly_ReplayIsDeterministic(t *testing.T) {
	t.Run("when the same stream is replayed twice then the logs are identical", func(t *testing.T) {
		first := entries(t, run(t, config.FeeReversalNone))
		second := entries(t, run(t, config.FeeReversalNone))
...
func TestAppendOnly_PostingOrderBeatsListedOrder(t *testing.T) {
	...
	t.Run("when the stream is shuffled then the result is unchanged", func(t *testing.T) {
		forward := run(t, config.FeeReversalNone)

		reversed := make([]entity.Event, 0, 10)
		stream := replay.CanonicalStream()
		for i := len(stream) - 1; i >= 0; i-- {
			reversed = append(reversed, stream[i])
		}
		shuffled := newStack(t, config.FeeReversalNone, replay.CanonicalAccounts()...)
		assert.NoError(t, shuffled.replay.Run(context.Background(), reversed))
		...
		assertMoney(t,
			row(t, forward, replay.ACC001, 6).ClosingBalance,
			row(t, shuffled, replay.ACC001, 6).ClosingBalance,
