...
The module follows the boilerplate's convention: `repository.go` / `service.go`
...
`mock/`, and the suite in `test/` as an external `ledger_test` package.

Two deviations, both forced by the brief. The boilerplate's controller and route
files are gin HTTP handlers; a web layer is forbidden here, so they keep their
shape -- free functions taking `Service`, plus a `NewRoutes` that wires them --
with an `io.Writer` as the delivery target instead of a `*gin.Context`. And
there is no `app/model`: that layer holds the database representation, and with
no database the entity is the stored form.

```
main.go                      composition root: repository -> service -> routes
...
app/module/ledger/
  repository.go              Repository interface + in-memory append-only store
  service.go                 Service interface + service struct + NewService
  controller.go              free functions taking Service, one per operation
  route.go                   NewRoutes: wires the controllers to the writer
  ...
  engine.go                  Replay / CloseDay / Report
  stream.go                  the ten canonical events
  mock/                      generated by mockery
  test/                      the suite
